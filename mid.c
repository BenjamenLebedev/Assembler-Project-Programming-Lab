...
void * symbol_ctor (const void * copy) {
    struct symbol * new_p = malloc(sizeof(struct symbol));
    if(new_p == NULL) 
        return NULL;  
...
void * extern_ctor (const void * copy) {
    struct ext * new_p = malloc(sizeof(struct ext));
    if(new_p == NULL) 
        return NULL; 
...
/*     Vector externals_vector = new_vector(extern_ctor, extern_dtor);
    struct ext *extern1; */
    struct ext *extern1 = malloc(sizeof(struct ext));
